...
import {
  ...
  createGuestAuthorizationPayload,
  verifyAdmin,
  getRolesValue,
  verifyGuest,
} from '$/packages/application/service/auth';

describe('createAdminAuthorizationPayload', () => {
  it('should return auth payload', () => {
    expect(createAdminAuthorizationPayload({ id: 123, roles: [{ name: 'a', role: 'b' }] } as Admin)).toEqual({
      id: 123,
      roles: ['b'],
    });
  });
...
describe('createGuestAuthorizationPayload', () => {
  it('should return auth payload', () => {
    expect(createGuestAuthorizationPayload(123)).toEqual({ id: 123 });
  });
});

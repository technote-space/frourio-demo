...
describe('capturePaymentIntents', () => {
  it('should capture payment intents (captured)', async() => {
  ...
  it('should capture payment intents (not captured)', async() => {
    const mock = jest.fn(() => getPromiseLikeItem(true));
    expect(await capturePaymentIntents(new TestReservationRepository([
      getDummyReservationData(await getRoom(), await getGuest()),
    ]), new TestPaymentRepository(), new TestMailRepository(mock), {
      id: 1,
      code: 'test code',
      guestId: 1,
      guestEmail: 'test@example.com',

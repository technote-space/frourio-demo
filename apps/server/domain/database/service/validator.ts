...
import { IValidatable } from '$/domain/database/service/validatable';
import { IReservationRepository } from '$/domain/database/reservation';
...
class IsIdExistsConstraint implements ValidatorConstraintInterface {
  ...
  async validate(value: any, args: ValidationArguments) {
    ...
    const repository = args.constraints[0] as IValidatable;
    const item = await repository.getDelegate().findFirst({ where: { id: Number(value) } });
    if (item === null) {
      return false;
    }
...
export function IsIdExists(repository: IValidatable, fields?: readonly string[], checkBody?: (field: string, body: Record<string, any>) => boolean, validationOptions?: ValidationOptions) {
  return function(object: Object, propertyName: string) {
    registerDecorator({
      target: object.constructor,
      propertyName: propertyName,
      options: validationOptions,
      constraints: [repository, fields, checkBody],
...
class IsUniqueValueConstraint implements ValidatorConstraintInterface {
  async validate(value: any, args: ValidationArguments) {
    ...
    const item = await repository.getDelegate().findFirst({ where });
    return item === null;
  }
...
export function IsUniqueValue(repository: IValidatable, validationOptions?: ValidationOptions) {
  return function(object: Object, propertyName: string) {
    registerDecorator({
      target: object.constructor,
      propertyName: propertyName,
...
class IsReservableConstraint implements ValidatorConstraintInterface {
  ...
  private static buildWhere(checkin: Date, checkout: Date, data: any) {
    const where = {
      AND: [
        {
          checkin: {
  ...
  async validate(value: any, args: ValidationArguments) {
    const data = args.object as any;
    const notCheckGuest = !!args.constraints[1];
    ...
    const repository = args.constraints[0] as IReservationRepository;
    const reservation = await repository.find(undefined, {
      where: IsReservableConstraint.buildWhere(checkin, checkout, data),
...
export function IsReservable(repository: IReservationRepository, notCheckGuest?: boolean, validationOptions?: ValidationOptions) {
  return function(object: Object, propertyName: string) {
    registerDecorator({
      target: object.constructor,
      propertyName: propertyName,
      options: validationOptions,
      constraints: [repository, notCheckGuest],
...
class IsWithinLimitConstraint implements ValidatorConstraintInterface {
  ...
  async validate(value: any, args: ValidationArguments) {
    ...
    const repository = args.constraints[0] as IValidatable;
    const id = `${repository.getModelName()}Id`;
    const data = args.object as any;
    ...
    const item = await repository.getDelegate().findFirst({ where: { id: Number(data[id]) } });
    if (!item) {
      return false;
    }
...
export function IsWithinLimit(repository: IValidatable, field: string, validationOptions?: ValidationOptions) {
  return function(object: Object, propertyName: string) {
    registerDecorator({
      target: object.constructor,
      propertyName: propertyName,
